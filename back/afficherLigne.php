<?PHP
	...
	$ligneC=new LigneC();
	$listelignes=$ligneC->afficher_LigneCommande();
  ...
  <br>
		...
		<div >
            <table class="table table-striped table-dark">
				...
				<?PHP
				foreach($listelignes as $user){
			?>
				<tr>
					<td><?PHP echo $user['idLigne']; ?></td>
					<td><?PHP echo $user['Produit']; ?></td>
					<td><?PHP echo $user['idCommande']; ?></td>
					<td>
						<form method="POST" action="SupprimerLigne.php">
						<button type="submit" style="background-color:transparent; border-color:transparent;"> 
						<img src="https://img.icons8.com/color/48/000000/delete-forever.png"/>
                         </button>	
						<input type="hidden" value=<?PHP echo $user['idLigne']; ?> name="idLigne">
						</form>
					</td>
